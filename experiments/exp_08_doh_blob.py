"""EXP-08: DoH (Determinant of Hessian) blob 検出

skimage.feature.blob_doh を使う。LoG より高速で重複に強い。
supermacro（75 泡密集）での挙動を LoG と比較するのが主目的。

スイープ:
    threshold (blob の強さ): 0.005 / 0.010 / 0.020
    max_sigma: 50 / 100

Usage:
    uv run python experiments/exp_08_doh_blob.py
"""
...
OUTPUT_DIR = Path("data/outputs/exp-08")
...
PARAM_SETS = [
    {"threshold": 0.005, "max_sigma": 50},
    {"threshold": 0.005, "max_sigma": 100},
    {"threshold": 0.010, "max_sigma": 50},
    {"threshold": 0.010, "max_sigma": 100},
    {"threshold": 0.020, "max_sigma": 50},
    {"threshold": 0.020, "max_sigma": 100},
]
...
def draw_overlay(image: np.ndarray, blobs: np.ndarray) -> np.ndarray:
    ...
    for y, x, sigma in blobs:
        r = int(sigma * (2 ** 0.5))
        cv2.circle(debug, (int(x), int(y)), max(r, 4), (255, 180, 0), 2)
...
def main() -> None:
    ...
    for ps in PARAM_SETS:
        ...
        tag = f"thresh{int(thresh*1000):03d}_maxsig{max_sig}"
